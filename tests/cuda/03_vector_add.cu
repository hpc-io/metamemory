...
int main(int argc, char *argv[]) {
  ...
  metamem* meta_h_A = metamem_init(METAMEM_CUDA);
  metamem* meta_h_B = metamem_init(METAMEM_CUDA);
  metamem* meta_h_C = metamem_init(METAMEM_CUDA);

  meta_h_A->fn->alloc(meta_h_A, numElements, sizeof(float), MEM_CPU_GPU_MANAGED, (mem_type_t)-1);
  meta_h_B->fn->alloc(meta_h_B, numElements, sizeof(float), MEM_CPU_GPU_MANAGED, (mem_type_t)-1);
  meta_h_C->fn->alloc(meta_h_C, numElements, sizeof(float), MEM_CPU_GPU_MANAGED, (mem_type_t)-1);
  ...
  for (uint64_t i = 0; i < numElements; ++i) {
    if (fabs(h_A[i] + h_B[i] - h_C[i]) > 1e-5) {
      fprintf(stderr, "Result verification failed at element %ld (%f)!\n", i, h_C[i]);
      return EXIT_FAILURE;
    }
  }
